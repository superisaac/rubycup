...
class ObserverBehaviour < Behaviour
  def initialize
    @state  = 0
  end
  def action
    ...
    if @state == 0
      ...
      @agent.receive_callback(ps, MessageSelector.match_performative(ACLMessage.INFORM))
      @state = 1
    end
  end  
end
...
class ObserverAgent < Agent
  ...
  def setup
    t = ui_run    
    add_behaviour(ObserverBehaviour.new)
  end
end
...
def test_observer
  ...
  p = Platform.new("Playground", "localhost:7778")

  s = p.start_service { |p|
    p.add_agent(ObserverAgent.new("Observer", p)) 
  } 
  s.join
end
test_observer
